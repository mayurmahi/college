      ...
      <div className="relative flex justify-between items-center w-full">
        ...
        {(router.pathname === "/faculty" || router.pathname === "/admin") &&
          (open ? (
            ...
            <button
              className="absolute right-2 flex justify-center items-center border-2 border-green-500 px-3 py-2 rounded text-green-500"
              onClick={openHandler}
            >
              Add Notice
              <span className="ml-2">
                <IoAddOutline className="text-green-500 text-xl" />
              </span>
            </button>
          ))}
      </div>
          ...
          {notice &&
            notice.map((item, index) => {
              return (
                <div
                  key={item._id}
                  className="border-green-500 border-2 w-full rounded-md shadow-sm py-4 px-6 mb-4 relative bg-gray-800"
                >
                  {(router.pathname === "/faculty" ||
                    ...
                    <div className="absolute flex justify-center items-center right-4 bottom-3">
                      ...
                      <span
                        className="text-2xl group-hover:text-green-500 ml-2 cursor-pointer hover:text-red-500"
                  ...
                  <p className="text-base font-normal mt-1 text-white">
                    {item.description}
                  </p>
                  <p className="text-sm absolute top-4 right-4 flex justify-center items-center text-white">
        ...
        <form className="mt-8 w-full">
          <div className="w-[40%] mt-2">
            ...
            <input
              type="text"
              id="title"
              className="bg-gray-700 py-2 px-4 w-full mt-1 text-white"
              value={data.title}
              onChange={(e) => setData({ ...data, title: e.target.value })}
            />
          </div>
          <div className="w-[40%] mt-4">
            <label htmlFor="title" className="text-white">Notice Description</label>
            <textarea
              id="title"
              className="bg-gray-700 py-2 px-4 w-full mt-1 h-[130px] text-white"
          ...
          <div className="w-[40%] mt-4">
            <label htmlFor="type" className="text-white">Notice Type</label>
            <select
              id="type"
              className="bg-gray-700 py-2 px-4 w-full mt-1 text-white"
              value={data.type}
